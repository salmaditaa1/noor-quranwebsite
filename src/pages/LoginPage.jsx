  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <div className="bg-noor-card border border-noor-divider shadow-noor-heavy rounded-noor p-8 w-full max-w-md relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-noor-gold/10 rounded-full blur-3xl"></div>
        <div className="absolute bottom-0 left-0 w-32 h-32 bg-noor-dark/10 rounded-full blur-3xl"></div>
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <h2 className="text-2xl font-extrabold text-noor-dark">Selamat Datang Kembali</h2>
          <p className="text-sm text-noor-textSecondary mt-2">Masuk untuk melanjutkan aktivitas ibadah Anda.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
          ...
          <button 
            type="submit"
            className="w-full py-3 bg-gradient-to-r from-noor-dark to-noor-light text-white font-bold rounded-xl shadow-md hover:shadow-lg hover:scale-[1.02] transition-all"
          >
            Masuk
          </button>
        </form>
        ...
        <p className="text-center text-xs font-semibold text-noor-textSecondary mt-6 relative z-10">
          Belum punya akun? <Link to="/register" className="text-noor-gold hover:underline font-bold">Daftar sekarang</Link>
        </p>
      </div>
    </div>
  );
